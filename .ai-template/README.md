...
| **2. Registro**   | agente gerador             | respostas                        | `.ai/memory/memory.json`  |
| **3. Geração**    | agente gerador             | templates + `memory/memory.json` | arquivos de `.ai/`        |
| **4. Validação**  | agente gerador             | `.ai/`                           | relatório de consistência |
| **5. Uso**        | agentes de desenvolvimento | `.ai/rules/RULES.md`             | código, evidências        |
...
| `memory/memory.json`               | `.ai/memory/memory.json`                | 1                                               |
| `rules/RULES.md`                   | `.ai/rules/RULES.md`                    | 1                                               |
...
| ---------------------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `rules/RULES.md`                   | Como o agente se comporta na sessão, o que pode decidir sozinho e o que exige aprovação | Detalhe de tecnologia, comandos de validação              |
...
| `memory/memory.json`               | Respostas da descoberta em formato legível por máquina                                  | Prosa normativa                                           |
...
| `{{campo.do.memory}}`      | Valor vindo de `memory/memory.json`                    | **Não** (é substituído)         |
...
| ------- | ---------------- | ------------------- | ------------- |
| `RULE`  | `rules/RULES.md` | `RULE-<GRUPO>-<NN>` | `RULE-GIT-03` |
...
4. **Escrever `.ai/memory/memory.json`** com as respostas confirmadas antes de gerar qualquer
   documento em prosa. `memory/memory.json` é a única fonte de verdade da geração.
5. **Gerar os arquivos** nesta ordem, porque cada um referencia os anteriores:
   `rules/RULES.md` → `skeletons/` → `guardrails/` → `gates/`.
...
8. **Referenciar `.ai/` no arquivo raiz de instrução do agente do projeto** (`CLAUDE.md`,
   ...
   arquivos existe no repositório e adicionar uma linha apontando para `.ai/rules/RULES.md`
...
- Quando uma informação necessária não existir: registrar em `memory/memory.json` (campo
...
- `memory/memory.json` (campo `framework.template_version`) registra a versão do template
...
- [ ] `memory/memory.json` é JSON válido e coerente com os documentos gerados.
- [ ] `.ai/rules/RULES.md` está referenciado no arquivo raiz de instrução do agente do
...
| #   | Ambiguidade                                                                       | Decisão                                                                                                                                                                                             |
| --- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `ask/questions.md` seria o índice do `.ai/` ou o roteiro de perguntas?            | É **exclusivamente o roteiro de descoberta** e não é copiado. O índice do `.ai/` é a seção _Mapa de contexto_ de `rules/RULES.md`.                                                                   |
| 2   | Qual arquivo é o ponto de entrada do `.ai/`?                                      | `.ai/rules/RULES.md`. Todo o resto é alcançável por referência `@` a partir dele.                                                                                                                    |
